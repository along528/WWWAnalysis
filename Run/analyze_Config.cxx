...
void analyze_Config(const std::string &ele_config, const std::string &mu_config, const std::string &event_config,const std::string &submit,const std::string &scandir="",const std::string &sysflag="")
{
...
*/
  if(sysflag=="All") {
  	sysconfig->switchOnAll();
  	sysconfig->switchOnCategory("MxM",false);
  }
  else if(sysflag=="") {
	...
	sysconfig->switchOnCategory("MissingEtScale");
	sysconfig->switchOnCategory("MissingEtReso");
  }
  else {
  ...
  */
  ...
  while(alg->m_pSystematics->next()){
	  ...
	  //alg->addObjectSelector("ElectronMxMTight" ,static_cast<wwwAnalysis::TElectronSelectorTool*>(ConfigureTool("TElectronSelectorTool","MxMTight")), var);
	  alg->addObjectSelector("ElectronLoose" ,static_cast<wwwAnalysis::TElectronSelectorTool*>(ConfigureTool("TElectronSelectorTool","Loose_InvertedID_InvertedEtIso_InvertedPtIso")), var);
	  alg->addObjectSelector("MuonLoose",static_cast<wwwAnalysis::TMuonSelectorTool*>(ConfigureTool("TMuonSelectorTool","Loose_InvertedEtIso_InvertedPtIso")),var);
	  //alg->addObjectSelector("MuonMxMTight",static_cast<wwwAnalysis::TMuonSelectorTool*>(ConfigureTool("TMuonSelectorTool","MxMTight")),var);
...
////Uncomment these two for fake selection:
...
//  wwwAnalysis::TMuonSelectorTool* fakeMuonSelectorTool = static_cast<wwwAnalysis::TMuonSelectorTool*>(ConfigureTool("TMuonSelectorTool","Fake"));
  ...
  alg->m_hPileupToolUp->AddConfigFile((path+"/pileup/sm_ew_www.prw.root").Data());
  alg->m_hPileupToolUp->SetDataScaleFactors(1.1/1.09);
  alg->m_hPileupToolUp->AddLumiCalcFile((path+"/pileup/sm_ew_ilumicalc.root").Data());
  alg->m_hPileupToolUp->SetUnrepresentedDataAction(2);

  alg->m_hPileupToolDown=new Root::TPileupReweighting("PURWTOOLDOWN");
  alg->m_hPileupToolDown->AddConfigFile((path+"/pileup/sm_ew_www.prw.root").Data());
  alg->m_hPileupToolDown->SetDataScaleFactors(0.9/1.09);
  alg->m_hPileupToolDown->AddLumiCalcFile((path+"/pileup/sm_ew_ilumicalc.root").Data());
  alg->m_hPileupToolDown->SetUnrepresentedDataAction(2);


  alg->setApplyUnitarization(true);
...
//EL::GEDriver driver;
//job.options()->setString (EL::Job::optSubmitFlags, "-V");
